...
check("cache refusals preserve exact bytes and make zero calls", local({
  ...
  withr_options(Rjif.transport = tr, {
    ...
    stopifnot(fp$version == 8L)   # v8: identity digests built from the serializer's own output (r6f); v7 canonicalized model/question digests (r6e); v6 added the model_id digest entry (r6c R6c-B1); v5 was per-row provenance columns (round 3 carryover 1); v4 was the UTF-8 digest (R3-1)
